...
const urlCoinMarketCrypto = "https://pro-api.coinmarketcap.com/v2/cryptocurrency/info?"
...
getData = async (url = '', apiKey='') => {
 ...
 (async (req,res) => {
    try {
        ...
        const x = json.data
        for (val in x){ 
            dataFrame.push((({ name, symbol, logo, date_launched}) => ({ name, symbol, logo, date_launched }))(x[val][0]))
        };
        CryptoInfo.remove({}, function(err) {console.log('collection removed') });
